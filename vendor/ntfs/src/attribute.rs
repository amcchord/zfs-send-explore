...
impl<'n, 'f> NtfsAttribute<'n, 'f> {
    ...
    /// Returns the complete attribute record after NTFS sector fixups.
    /// This exposes compression metadata not otherwise available in 0.4.0.
    pub fn record_data(&self) -> &'f [u8] {
        &self.file.record_data()[self.offset..self.offset + self.attribute_length() as usize]
    }
